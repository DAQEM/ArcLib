...
public class CreateExplosionReward extends AbstractReward {
    ...
    public CreateExplosionReward(double chance, int priority, float radius, boolean causesFire, Level.ExplosionInteraction blockInteraction) {
    ...
    public Component getDescription() {
        return getDescription(radius, causesFire, blockInteraction.name());
    }

    @Override
    public ActionResult apply(ActionData actionData) {
        Level level = actionData.getPlayer().arc$getLevel();
        level.explode(null, actionData.getPlayer().arc$getPlayer().getX(), actionData.getPlayer().arc$getPlayer().getY(), actionData.getPlayer().arc$getPlayer().getZ(), radius, causesFire, blockInteraction);
    ...
    public static class Serializer implements IRewardSerializer<CreateExplosionReward> {
        ...
        public CreateExplosionReward fromJson(JsonObject jsonObject, double chance, int priority) {
            return new CreateExplosionReward(
                    chance,
                    priority,
                    GsonHelper.getAsFloat(jsonObject, "radius"),
        ...
        public void toNetwork(RegistryFriendlyByteBuf friendlyByteBuf, CreateExplosionReward type) {
            ...
            friendlyByteBuf.writeFloat(type.radius);
